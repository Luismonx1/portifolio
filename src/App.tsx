...
export default function App() {
  ...
  return <>
    ...
    <main id="conteudo" tabIndex={-1}>
      ...
      <section id="contato" className="container contact-section"><div className="contact-card"><div><span className="section-label">06 — VAMOS CONVERSAR?</span><h2>O próximo passo<br />{' '}pode começar com um <span>olá.</span></h2><p>Busco minha primeira oportunidade em análise de dados.<br />{' '}Se você tem uma oportunidade ou uma ideia, vamos conversar.</p><a className="button button-primary" href={`mailto:${profile.email}`}><Mail size={18} /> Entre em contato <ArrowUpRight size={17} /></a></div><div className="contact-links"><a href={profile.whatsapp} target="_blank" rel="noreferrer"><MessageCircle /><span><small>WHATSAPP</small>{profile.whatsappLabel}</span><ArrowUpRight size={18} /></a><a href={`mailto:${profile.email}`}><Mail /><span><small>E-MAIL</small>{profile.email}</span><ArrowUpRight size={18} /></a><a href={profile.linkedin} target="_blank" rel="noreferrer"><Linkedin /><span><small>LINKEDIN</small>Vamos nos conectar</span><ArrowUpRight size={18} /></a><a href={profile.github} target="_blank" rel="noreferrer"><Github /><span><small>GITHUB</small>@Luismonx1</span><ArrowUpRight size={18} /></a></div></div></section>
    </main>
